...
| 2026-09-13 | Сверка state ⇄ коллекция: признак «проиндексирован» — id-префикс, а не `path`; `upsert` вместо `insert` (скрытые `doc_id`); самопроверка в прогоне и в `zvec-index.sh`; режим `--cleanup-orphans` |
...
| `indexer.py` | Инкрементальный индексатор (SHA1-хэши файлов), сверка state ⇄ коллекция, `optimize()` после каждого прогона |
...
## Как индексатор понимает, что файл уже проиндексирован

`state.json` хранит только хэш файла. Хэш говорит «файл не менялся», но не говорит,
что его чанки доступны в коллекции, — а расходиться это может тремя способами:

| Расхождение | Как выглядит |
|---|---|
| чанки записаны старым (до-хэшевым) форматом id | в коллекции лежит устаревший текст по тому же `path` |
| чанки «скрыты» | `upsert` по `doc_id` отвечает `already exists`, но `iter_docs`/`delete_by_filter` этот док не видят |
| чанки не создавались | в коллекции нет ничего по этому `path` |

Поэтому признак «проиндексирован» — не `path`, а **id-префикс** файла
(`source-<sha1(source:rel)[:12]>-`): он строится в `make_doc_id` и не зависит от
формата хранения. Файл с совпавшим хэшем, но без живого префикса, переиндексируется
через `upsert` (не `insert`: `insert` на скрытый `doc_id` молча теряет чанк, а `state`
уже отмечен — файл остаётся «проиндексированным, но недоступным» навсегда).

Каждый прогон заканчивается самопроверкой: `ВНИМАНИЕ: без актуальных чанков осталось N`.
Обёртка `zvec-index.sh` считает эту строку ошибкой прогона (код 1), так что «тихий»
недоиндексированный файл больше не проходит незамеченным.

Осиротевшие чанки (пути, которых нет в источниках) удаляются отдельным режимом —
намеренно НЕ автоматически: при временно недоступном источнике (не смонтирован
каталог, нет прав) автоочистка снесла бы живой индекс.

```bash
.venv/bin/python indexer.py --cleanup-orphans    # удалить чанки по мёртвым путям
```
